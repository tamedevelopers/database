...
abstract class Insertion extends Builder {
    ...
    public function optimize(string|array $table = [])
    {
        ...
        // filter array
        $this->table = $this->arrayWalkerTrim($this->table);
        ...
        if($analize['status'] !== Constant::STATUS_200){
            return $analize;
        }

        // repair
        $repair = $this->repairTable();
        if($repair['status'] !== Constant::STATUS_200){
            return $repair;
        }
    ...
    public function tableExists(?string $table_name = null)
    {
        try{
            ...
            if($this->dbConnection('status') != Constant::STATUS_200){
                return false;
            }
    ...
    public function exists()
    {
        if($this->modelQuery){
            ...
            try {
                ...
                return $count >= Constant::ONE ? true : false;
            } catch (\PDOException $e) {
                return false;
            }
        }
    ...
    public function increment(?string $column, $count = 1, $param = [])
    {
        if($this->modelQuery){
            // operator
            $temp  = $this->configIncrementOperator($column, $count, $param);
